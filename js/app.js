...
function filtrarYMostrarProductos(termino) {
    ...
    if(filtrados.length === 0) { cont.innerHTML = `<p class="text-center text-[10px] text-slate-400 mt-4 italic uppercase">Sin artículos</p>`; return; }
    filtrados.forEach(p => {
...
function setupEvents() {
    ...
    document.getElementById('form-db')?.addEventListener('submit', async (e) => {
        ...
        if (id) { if(await db.actualizarProducto(id, p)) { alert("ACTUALIZADO"); limpiarFormDB(); } } 
        else { if(await db.guardarNuevoProducto(p)) { alert("GUARDADO"); e.target.reset(); } }
    });
        ...
        ui.startScanner(async (text) => {
            ...
            if(p) {
                ...
                if(op) window.agregarProdAlCarrito(text, p.nombre, p.venta);
                    ...
                    const cant = prompt(`¿Piezas que entran?`, "1");
    ...
    snap.forEach(docSnap => {
        ...
        cont.innerHTML += `<div class="p-3 bg-white border rounded-xl mb-2 shadow-sm border-l-4 ${d.tipoImpresion === 'laser_bn' ? 'border-slate-400':'border-indigo-500'}">
            ...
            <div class="flex gap-1">
                ...
                <a href="${d.archivoURL}" target="_blank" class="bg-slate-900 text-white px-2 py-1 rounded-lg text-[9px] font-bold text-center flex items-center px-2 text-xs">VER</a>
            </div></div>`;
    });
}
...
function renderBitacoraImpresiones(snap) {
    ...
    snap.forEach(docSnap => { const d = docSnap.data(); tot += d.monto || 0; const f = d.fecha ? new Date(d.fecha.seconds * 1000).toLocaleDateString('es-MX', {hour:'2-digit', minute:'2-digit'}) : '...'; tbody.innerHTML += `<tr class="border-b hover:bg-indigo-50"><td class="p-3 text-slate-500">${f}</td><td class="p-3 font-bold uppercase text-left">${d.usuario}</td><td class="p-3 text-center"><span class="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full text-[8px] font-black uppercase">${d.servicio}</span></td><td class="p-3 text-center font-bold">${d.paginas}</td><td class="p-3 text-right font-black text-green-600">$${(d.monto || 0).toFixed(2)}</td></tr>`; });
...
window.cancelarArchivo = async (id) => { if(confirm("¿Borrar archivo?")) await db.eliminarRegistro('cola_impresion', id); };
...
window.eliminarProducto = async (id) => { if(confirm("¿Eliminar definitivamente?")) await db.eliminarRegistro('inventario', id); };
...
window.verEtiqueta = (id, nombre, precio) => {
    ...
    document.getElementById('etiqueta-precio').innerText = `$${(precio || 0).toFixed(2)}`;
    const qrCont = document.getElementById('etiqueta-qr'); if(qrCont) { qrCont.innerHTML = ""; new QRCode(qrCont, { text: id, width: 150, height: 150 }); }
...
function limpiarFormDB() { document.getElementById('db-id').value = ""; document.getElementById('form-db').reset(); const btn = document.getElementById('btn-guardar-prod'); btn.innerText = "Añadir Producto"; btn.classList.replace('bg-amber-500', 'bg-indigo-600'); document.getElementById('btn-cancelar-edicion').classList.add('hidden'); }
